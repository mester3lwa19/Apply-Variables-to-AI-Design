...
function isMixed(val) {
  return typeof val === "symbol";
}
function safeArray(val) {
  return !val || isMixed(val) || !Array.isArray(val) ? [] : val;
}
...
function colorKey(paint) {
  ...
  var opacity =
    paint.opacity !== undefined && !isMixed(paint.opacity) ? paint.opacity : 1;
...
function fontKey(node) {
  ...
  var lhVal =
    node.lineHeight && !isMixed(node.lineHeight) && node.lineHeight.value
      ? Math.round(node.lineHeight.value * 10) / 10
      : 0;
  var lsVal =
    node.letterSpacing &&
    !isMixed(node.letterSpacing) &&
    node.letterSpacing.value
      ? Math.round(node.letterSpacing.value * 10) / 10
      : 0;
  return [family, style, Math.round(size), lhVal, lsVal].join("|");
}

function safeId(val) {
  return !val || isMixed(val) || typeof val === "symbol"
    ? null
    : typeof val === "string"
      ? val
      : null;
  ...
  try {
    ...
    if (options.syncPaint) {
      ...
      for (var p = 0; p < localPaint.length; p++) {
        ...
        if (solid) {
          var op =
            solid.opacity !== undefined && !isMixed(solid.opacity)
              ? solid.opacity
              : 1;
          paintCache.push({
            name: ps.name,
            key: ps.key,
            hex: hexFromPaint(solid),
    ...
    await figma.clientStorage.setAsync("token_mapper_sync", {
...
function scanDocument() {
  var textGroups = {},
    colorGroups = {},
    referencedTextStyleIds = {},
    referencedPaintStyleIds = {},
  ...
  if (selection.length > 0) {
    for (var n = 0; n < selection.length; n++) {
      ...
      if ("findAll" in selection[n])
        allNodes = allNodes.concat(selection[n].findAll());
    }
  } else {
    try {
  ...
  for (var i = 0; i < allNodes.length; i++) {
    ...
    if (node.boundVariables) {
      if (node.boundVariables.fills) {
        for (var b = 0; b < node.boundVariables.fills.length; b++) {
          if (
            node.boundVariables.fills[b] &&
            node.boundVariables.fills[b].type === "VARIABLE_ALIAS"
          )
            referencedVariableIds[node.boundVariables.fills[b].id] = true;
        }
      }
      if (node.boundVariables.strokes) {
        for (var s = 0; s < node.boundVariables.strokes.length; s++) {
          if (
            node.boundVariables.strokes[s] &&
            node.boundVariables.strokes[s].type === "VARIABLE_ALIAS"
          )
        ...
        if (fn) {
          ...
          if (!textGroups[key]) {
            ...
            textGroups[key] = {
              ...
              letterSpacing: !isMixed(node.letterSpacing)
                ? node.letterSpacing
                : null,
              sample:
                chars && typeof chars === "string" ? chars.slice(0, 40) : "",
              count: 0,
              nodeIds: [],
    ...
    for (var f = 0; f < fills.length; f++) {
      if (!fills[f] || fills[f].type !== "SOLID" || fills[f].visible === false)
        continue;
      try {
        var ck = colorKey(fills[f]);
        if (ck) {
          var boundVarId =
            fills[f].boundVariables &&
            fills[f].boundVariables.color &&
            fills[f].boundVariables.color.type === "VARIABLE_ALIAS"
              ? fills[f].boundVariables.color.id
              : null;
          var fsId = safeId(node.fillStyleId);
          if (!colorGroups[ck]) {
            var fillOp =
              fills[f].opacity !== undefined && !isMixed(fills[f].opacity)
                ? fills[f].opacity
                : 1;
            colorGroups[ck] = {
              id: ck,
              hex: hexFromPaint(fills[f]),
          ...
          colorGroups[ck].nodeTypes[node.type] =
            (colorGroups[ck].nodeTypes[node.type] || 0) + 1;
          if (boundVarId) {
            referencedVariableIds[boundVarId] = true;
            colorGroups[ck].existingVarId = boundVarId;
          } else if (fsId) {
            referencedPaintStyleIds[fsId] = true;
            colorGroups[ck].existingStyleId = fsId;
          }
        }
      } catch (e) {}
    }
    ...
    for (var st = 0; st < strokes.length; st++) {
      if (
        !strokes[st] ||
        strokes[st].type !== "SOLID" ||
        strokes[st].visible === false
      )
        continue;
      try {
        var sk = "stroke_" + colorKey(strokes[st]);
        var boundVarIdStroke =
          strokes[st].boundVariables &&
          strokes[st].boundVariables.color &&
          strokes[st].boundVariables.color.type === "VARIABLE_ALIAS"
            ? strokes[st].boundVariables.color.id
            : null;
        var ssId = safeId(node.strokeStyleId);
        if (!colorGroups[sk]) {
          var strokeOp =
            strokes[st].opacity !== undefined && !isMixed(strokes[st].opacity)
              ? strokes[st].opacity
              : 1;
          colorGroups[sk] = {
            id: sk,
            hex: hexFromPaint(strokes[st]),
        ...
        colorGroups[sk].nodeTypes["STROKE"] =
          (colorGroups[sk].nodeTypes["STROKE"] || 0) + 1;
        if (boundVarIdStroke) {
          referencedVariableIds[boundVarIdStroke] = true;
          colorGroups[sk].existingVarId = boundVarIdStroke;
        } else if (ssId) {
          referencedPaintStyleIds[ssId] = true;
          colorGroups[sk].existingStyleId = ssId;
        }
      } catch (e) {}
    }
  }
...
async function loadStyles(
  ...
  try {
    ...
    localText.forEach((s) => {
      var fn =
        s.fontName && !isMixed(s.fontName) && s.fontName.family
          ? s.fontName
          : null;
      textStyleMap[s.id] = {
        id: s.id,
        ...
        family: fn ? fn.family : "—",
        style: fn ? fn.style : "—",
        size: s.fontSize ? Math.round(s.fontSize) : 0,
        isLocal: true,
        isCached: false,
      };
    });
  ...
  try {
    var localPaint = await figma.getLocalPaintStylesAsync();
    localPaint.forEach((ps) => {
      var solid = safeArray(ps.paints).find((x) => x.type === "SOLID");
      var op = solid
        ? solid.opacity !== undefined && !isMixed(solid.opacity)
          ? solid.opacity
          : 1
  ...
  for (var j = 0; j < (referencedTextIds || []).length; j++) {
    if (!textStyleMap[referencedTextIds[j]]) {
      try {
        var ls = await figma.getStyleByIdAsync(referencedTextIds[j]);
        if (ls && ls.type === "TEXT") {
          var lfn =
            ls.fontName && !isMixed(ls.fontName) && ls.fontName.family
              ? ls.fontName
              : null;
          textStyleMap[ls.id] = {
            id: ls.id,
  ...
  for (var q = 0; q < (referencedPaintIds || []).length; q++) {
    if (!paintStyleMap[referencedPaintIds[q]]) {
      try {
        var lps = await figma.getStyleByIdAsync(referencedPaintIds[q]);
        if (lps && lps.type === "PAINT") {
          var lSolid = safeArray(lps.paints).find((x) => x.type === "SOLID");
          var lOp = lSolid
            ? lSolid.opacity !== undefined && !isMixed(lSolid.opacity)
              ? lSolid.opacity
              : 1
            : 1;
          paintStyleMap[lps.id] = {
            id: lps.id,
...
async function getOrImportStyle(payload, isPaint) {
  if (payload.isCached) {
    try {
      return await figma.importStyleByKeyAsync(payload.id);
    } catch (e) {
      throw new Error(
        "Cannot import library style. Ensure the library is published.",
      );
    }
  } else {
...
async function getOrImportVariable(payload) {
  if (payload.isCached) {
    try {
      return await figma.variables.importVariableByKeyAsync(payload.id);
    } catch (e) {
      throw new Error(
        "Cannot import library variable. Ensure the library is published.",
      );
    }
  } else {
      ...
      try {
        ...
        if (isStroke && "strokes" in node) {
          ...
          for (var s = 0; s < strokes.length; s++) {
            if (
              strokes[s].type === "SOLID" ||
              strokes[s].type === "VARIABLE_ALIAS"
            ) {
              strokes[s] = figma.variables.setBoundVariableForPaint(
                strokes[s],
                "color",
                variable,
              );
          ...
          for (var f = 0; f < fills.length; f++) {
            if (
              fills[f].type === "SOLID" ||
              fills[f].type === "VARIABLE_ALIAS"
            ) {
              fills[f] = figma.variables.setBoundVariableForPaint(
                fills[f],
                "color",
                variable,
              );
...
figma.ui.onmessage = async function (msg) {
  try {
    if (msg.type === "GET_SYNC_OPTIONS") {
      ...
      var styles = await loadStyles(
        scan.referencedTextStyleIds,
        scan.referencedPaintStyleIds,
        scan.referencedVariableIds,
      );
      figma.ui.postMessage({
